...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    session's scope before dispatching.
    """
    from toolsets import is_tool_in_enabled_toolsets
    from tools.tool_search import TOOL_SEARCH_NAME, TOOL_DESCRIBE_NAME, TOOL_CALL_NAME
    bridge = {TOOL_SEARCH_NAME, TOOL_DESCRIBE_NAME, TOOL_CALL_NAME}
    def _consider(name: str) -> bool:
        if name in bridge:
            return False
        return is_tool_in_enabled_toolsets(name, agent.enabled_toolsets)
    tool_names = frozenset(n for n in agent._tool_search_catalog if _consider(n))
    logger.debug("tool_search scoped names (len=%s)", len(tool_names))
    return tool_names
